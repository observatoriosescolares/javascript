...
var url = "https://iot.educa.madrid.org/ServiceJson/JsonSensorObservations?proveedorID="+proveedor+"&sensorID="+parametro+"&rango="+rango+"&fin=-1";
...
function crearGrafica(myArr1){
    ...
    if (!maximoURL){maximoGRA = maximo+maximo*0.02}else{maximoGRA=parseFloat(maximoURL)};
    if (!minimoURL){minimoGRA = minimo-minimo*0.02}else{minimoGRA=parseFloat(minimoURL)};
    ...
    plot6 = $.jqplot("parametro",  [myArr1], {	
		...
		xaxis:{ min:(finGra-(3600*rango*1000)), max: finGra, renderer: $.jqplot.DateAxisRenderer,
			...
			label:'max: ' +maximo+ ', min: ' +minimo+ ', media: '+ media.toFixed(1) + ' ultima medida: ' + ((Date.now() - finGra)/1000).toFixed(0) + 's'
                },
